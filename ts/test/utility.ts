...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.JOKEDELIVERY_TEST_LIVE ||
    'TRUE' === process.env.JOKEDELIVERY_TEST_OVERRIDE
  ) {
  ...
  m.JOKEDELIVERY_TEST_EXPLAIN = process.env.JOKEDELIVERY_TEST_EXPLAIN || m.JOKEDELIVERY_TEST_EXPLAIN
