...
//     (\`new JokeDeliverySDK(...)\` / \`JokeDeliverySDK.test(...)\` become
//     \`JokeDeliverySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'JokeDeliverySDK'
...
const TEST_SEED = {"entity":{"random_joke":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new JokeDeliverySDK(...)\` and any
// \`JokeDeliverySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+JokeDeliverySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/JokeDeliverySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
