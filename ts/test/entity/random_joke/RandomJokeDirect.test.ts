...
describe('RandomJokeDirect', async () => {
  ...
  test('direct-load-random_joke', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-random_joke', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['JOKE_DELIVERY_TEST_RANDOM_JOKE_ENTID']
