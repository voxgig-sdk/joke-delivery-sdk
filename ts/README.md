# JokeDelivery TypeScript SDK



The TypeScript SDK for the JokeDelivery API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.RandomJoke()` — each with a small set of operations (`load`)
...
```ts
import { JokeDeliverySDK } from '@voxgig-sdk/joke-delivery-sdk'
...
| `tester(testopts?, sdkopts?)` | `JokeDeliverySDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `JokeDeliverySDK.test(testopts?, sdkopts?)` | `JokeDeliverySDK` | Create a test-mode client. |
...
| `client` | `client(): JokeDeliverySDK` | Return the parent SDK client. |
...
```ts
const random_joke = await client.RandomJoke().load({ id: 1 })
```
...
joke-delivery/
├── src/
│   ├── JokeDeliverySDK.ts        # Main SDK class
