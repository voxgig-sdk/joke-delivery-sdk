...
from jokedelivery_sdk.jokedelivery_types import (
    RandomJoke,
    RandomJokeLoadMatch,
)
...
class RandomJokeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RandomJokeEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> RandomJoke:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RandomJoke().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
