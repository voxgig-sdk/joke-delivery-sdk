...
class JokeDeliveryTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JokeDeliveryTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JokeDeliveryTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JokeDeliveryTestRunner.getenv("JOKE_DELIVERY_TEST_LIVE")
        override = JokeDeliveryTestRunner.getenv("JOKE_DELIVERY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JokeDeliveryTestRunner.getenv(key)
        ...
        explain = JokeDeliveryTestRunner.getenv("JOKE_DELIVERY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if JokeDeliveryTestRunner._test_control is not None:
            return JokeDeliveryTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JokeDeliveryTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JokeDeliveryTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JokeDeliveryTestRunner.load_test_control()
...
def load_env_local():
    JokeDeliveryTestRunner.load_env_local()


def env_override(m):
    return JokeDeliveryTestRunner.env_override(m)


def entity_data(v):
    return JokeDeliveryTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JokeDeliveryTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JokeDeliveryTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JokeDeliveryTestRunner.load_test_control()


def live_client_options():
    return JokeDeliveryTestRunner.live_client_options()


def live_delay_ms():
    return JokeDeliveryTestRunner.live_delay_ms()
