...
describe("RandomJokeEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set JOKE_DELIVERY_TEST_RANDOM_JOKE_ENTID JSON to run live")
      return
    end
    ...
    assert.is_nil(err)
    local random_joke_ref01_data_dt0_load_result = helpers.to_map(type(random_joke_ref01_data_dt0_loaded) == 'table' and random_joke_ref01_data_dt0_loaded.data_get and random_joke_ref01_data_dt0_loaded:data_get() or random_joke_ref01_data_dt0_loaded)
...
function random_joke_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("JOKE_DELIVERY_TEST_RANDOM_JOKE_ENTID")
  ...
  if env["JOKE_DELIVERY_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["JOKE_DELIVERY_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
