...
local RandomJokeEntity = {}
RandomJokeEntity.__index = RandomJokeEntity


function RandomJokeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RandomJokeEntity)
  self._name = "random_joke"
  self._client = client
...
function RandomJokeEntity:get_name()
  return self._name
end


function RandomJokeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RandomJokeEntity.new(self._client, opts)
end
...
function RandomJokeEntity:mark_deleted()
  self._deleted = true
end


function RandomJokeEntity:deleted()
  return true == self._deleted
end


function RandomJokeEntity:data_set(args)
  if args ~= nil then
...
function RandomJokeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RandomJokeEntity:match_set(args)
  if args ~= nil then
...
function RandomJokeEntity:match_get()
...
function RandomJokeEntity:stream(action, args, callopts)
...
function RandomJokeEntity:_run_op(ctx, post_done)
  local utility = self._utility
