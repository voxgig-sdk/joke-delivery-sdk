...
MCP server exposing the JokeDelivery SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `joke-delivery_list` | `entity`, optional `query` map | First page of records as JSON |
| `joke-delivery_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `joke-delivery_list` and
`joke-delivery_load` tools become available in new sessions.
