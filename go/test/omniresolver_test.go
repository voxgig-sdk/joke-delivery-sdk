...
import (
	...
	omni "github.com/voxgig-sdk/joke-delivery-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.JokeDeliveryError); is {
				out := map[string]any{
					"name":    "JokeDeliveryError",
...
func omniCtx(arg any, client *sdk.JokeDeliverySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.JokeDeliverySDK); is {
			client = live
