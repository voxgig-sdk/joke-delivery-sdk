...
class JokeDeliveryMakeError
{
    public static function call(?JokeDeliveryContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new JokeDeliveryContext([], null);
        }
        $op = $ctx->op ?? new JokeDeliveryOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new JokeDeliveryResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof JokeDeliveryError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new JokeDeliveryError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof JokeDeliveryError) {
