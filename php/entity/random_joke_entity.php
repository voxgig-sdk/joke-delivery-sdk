...
// JokeDelivery SDK RandomJoke entity
...
class RandomJokeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new RandomJokeEntity($this->_client, $opts);
    }

    /**
     * @param RandomJoke|array $args RandomJoke data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = JokeDeliveryHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return RandomJoke|array The current RandomJoke data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of RandomJoke fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = JokeDeliveryHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of RandomJoke fields).
     */
    ...
    /**
     * Load a single RandomJoke.
     *
     * @param RandomJokeLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed RandomJokeLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return RandomJoke|array The loaded RandomJoke as an assoc-array at the
     *   SDK boundary; throws JokeDeliveryError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = JokeDeliveryHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
