...
// JokeDelivery SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class JokeDeliveryTestFeature extends JokeDeliveryBaseFeature
{
    ...
    public function init(JokeDeliveryContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (JokeDeliveryContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = JokeDeliveryParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (JokeDeliveryContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(JokeDeliveryContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = JokeDeliveryParam::call($ctx, $k);
