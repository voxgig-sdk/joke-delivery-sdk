# JokeDelivery SDK utility: make_request
...
module JokeDeliveryUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = JokeDeliveryResponse.new({})
    ...
    if fetch_err
      ...
      response = JokeDeliveryResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = JokeDeliveryResponse.new(fetched)
    else
