...
// JokeDelivery SDK retry feature
...
class JokeDeliveryRetryFeature extends JokeDeliveryBaseFeature
{
    ...
    public function init(JokeDeliveryContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (JokeDeliveryContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(JokeDeliveryContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(JokeDeliveryContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
