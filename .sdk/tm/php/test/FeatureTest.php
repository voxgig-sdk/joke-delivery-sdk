...
class FtCtrl extends JokeDeliveryControl
{
...
class FtClient
{
    ...
    private JokeDeliveryUtility $utility;

    public function __construct(JokeDeliveryUtility $utility, array $options)
    {
    ...
    public function get_utility(): JokeDeliveryUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JokeDeliveryContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public JokeDeliveryContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (JokeDeliveryContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JokeDeliveryConfig::make_feature($name);
    ...
    private static function buildUrl(JokeDeliverySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JokeDeliveryError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JokeDeliveryResponse($fetched);
        }
    ...
    private function fail(JokeDeliveryContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JokeDeliveryContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
