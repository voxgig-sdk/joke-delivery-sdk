...
module JokeDeliveryOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if JokeDeliveryOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = JokeDeliveryOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| JokeDeliveryOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || JokeDeliveryOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = JokeDeliverySpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = JokeDeliveryError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { JokeDeliveryOmni.errify(err) }
