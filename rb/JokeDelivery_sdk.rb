...
require_relative 'JokeDelivery_types'
...
class JokeDeliverySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = JokeDeliveryHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = JokeDeliveryHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, JokeDeliveryFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    JokeDeliveryUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = JokeDeliveryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = JokeDeliveryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = JokeDeliveryHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = JokeDeliveryHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = JokeDeliveryError.new(
        "graphql_error", "JokeDeliverySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = JokeDeliverySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
