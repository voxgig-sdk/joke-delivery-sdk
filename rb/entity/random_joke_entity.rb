...
class RandomJokeEntity
  ...
  def make
    opts = @_entopts.dup
    RandomJokeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = JokeDeliveryHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [RandomJoke, Hash] the current RandomJoke data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = JokeDeliveryHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of RandomJoke fields)
  def match_get
  ...
  #
  # @param reqmatch [RandomJokeLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.RandomJoke.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [RandomJoke, Hash] the loaded RandomJoke; raises JokeDeliveryError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = JokeDeliveryHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
