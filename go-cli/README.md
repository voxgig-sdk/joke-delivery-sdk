...
boru-driven command-line client **and** interactive REPL for the JokeDelivery
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/joke-delivery-cli)
make build

# 2. See usage (words, entities, env vars)
./joke-delivery-cli --help

# 3. Provide credentials once, via the environment
export JOKE_DELIVERY_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./joke-delivery-cli load 1 random_joke            # {id:1} shorthand
./joke-delivery-cli load '{id:1}' random_joke       # explicit match map

# 5. Override the API base URL for a single call
JOKE_DELIVERY_BASE=https://api.example.com ./joke-delivery-cli load 1 random_joke

# 6. No arguments -> interactive REPL
./joke-delivery-cli
joke-delivery> load 1 random_joke
joke-delivery> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/joke-delivery-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/joke-delivery-cli load 1 random_joke
   ```
...
```sh
./joke-delivery-cli load 1 random_joke          # scalar shorthand for {id:1}
./joke-delivery-cli load '{id:1}' random_joke     # explicit match map
```
...
./joke-delivery-cli load 1 random_joke
```
...
Run with no arguments to open a REPL (prompt `joke-delivery>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `random_joke`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/joke-delivery-cli`. |
...
(`./joke-delivery-cli <expr>`) and interactively (the REPL), and expressions compose the
